...
func play_end_screen_sequence() -> void:
	...
	_animation_tween.tween_property(self, "velocity:x", 100.0, 0.0)
	_animation_tween.tween_interval(1.05)
	_animation_tween.tween_property(self, "velocity:x", 0.0, 0.0)
	_animation_tween.tween_interval(0.2)
	_animation_tween.tween_callback(SoundPool.play_sound.bind(SoundPool.SOUND_PLAYER_JUMP))
	_animation_tween.tween_callback(_set_velocity_y.bind(-140.0))
	_animation_tween.tween_interval(0.45)
	_animation_tween.tween_callback(SoundPool.play_sound.bind(SoundPool.SOUND_FIRE_SHOT))
	_animation_tween.tween_callback(_spawn_fire_ball)
	_animation_tween.tween_interval(0.25)
	_animation_tween.tween_callback(SoundPool.play_sound.bind(SoundPool.SOUND_FIRE_SHOT))
	_animation_tween.tween_callback(_spawn_fire_ball)
	_animation_tween.tween_interval(1.0)
	_animation_tween.tween_callback(_set_animation_override.bind(AnimationOverrides.CHEER))
